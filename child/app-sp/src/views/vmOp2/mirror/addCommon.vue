<template>
    ...
    >
        <el-form :model="form_data" :rules="rules" ref="mirror_form" label-width="170px" class="demo-ruleForm" label-position="left" label-suffix=":">
            ...
            <el-form-item label="模型镜像类型" prop="model_image_type">
                <el-select v-model="form_data.model_image_type">
                    <el-option v-for="item in model_image_type_list" :key="item" :label="item" :value=" item "></el-option>
                </el-select>
            </el-form-item>
...
export default class AddCommon extends Vue{
    ...
    private model_image_type_list:any=[];
    ...
    private async get_model_image_type(){
        let res:any = await Service.get_model_image_type({})
        if(res.code==="Success"){
            this.model_image_type_list=res.data.type_list;
            if(!this.oper_info.os_id){
              this.form_data.model_image_type = this.model_image_type_list[0]
            }
        }
    }
...
//   }
    ...
    private async confirm(){
        ...
        const {product_source,display_name,os_type,os_version,os_bit,size,customer_ids,az_id,backend_type,model_image_type,support_type,support_gpu_driver,oss_file_name,os_file_type,path_md5,upload_time,core_version,driver_version,cuda_version,other_software,validity,vali_time,display_name_en}=this.form_data
            ...
            if(valid){
                if(this.oper_info.os_id){
                    let res:any = await Service.edit_pub_mirror({
                        ...
                        os_version,
                        os_bit,
                        model_image_type:model_image_type
                    })
                    if(res.code==='Success'){
