<template>
  <div>
    <action-block :search_option="search_dom" @fn-search="search" :type="true">
      <!-- <template #default>
    ...
    >
      ...
      <el-table-column prop="disk_id" label="云盘ID" min-width="150px" :show-overflow-tooltip="true"></el-table-column>
      <el-table-column prop="disk_name" label="云盘名称" min-width="100px" >
      ...
      <el-table-column prop="type" label="类型容量" min-width="120px">
        <template slot-scope="scope">
      ...
      <el-table-column prop="az_name" label="地域及可用区" min-width="130px">
          <template slot-scope="scope">
      ...
      <el-table-column prop="ecs_id" label="实例ID" min-width="140px" :show-overflow-tooltip="true">
        <template slot-scope="scope">
      ...
      <el-table-column prop="product_source" label="产品来源" :filter-multiple="true" :filters="product_source_list" column-key="product_source">
        <template slot-scope="scope">
          <span v-for="item in product_source_list">
            <span v-if="item.value === scope.row.product_source">
              {{ item.text }}
            </span>
          </span>
        </template>
...
<!--      </el-table-column>-->
    ...
    <span v-if="mount_id.length===1">
        <right-click :multi_rows="mount_id" :menus="menus" :name=" mount_id.length>0 ? mount_id[0].disk_name: ''" :error_msg="error_msg"  @fn-click="infoClick"></right-click>
      </span>
  </div>
</template>
...
import MountDisk from '../../disk/mountDisk.vue';
import UnInstall from '../../disk/unInstall.vue';
import EditAttr from '../../disk/editAttr.vue';
import EditName from '../../disk/editName.vue';
import Common from '../../disk/commonDialog.vue'
...
export default class extends Vue {
  ...
  private infoClick(item) {
    this.operateRecord(this.mount_id)
    hideMenu()
  }
