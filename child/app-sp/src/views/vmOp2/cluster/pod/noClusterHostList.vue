...
<!--        </el-tooltip>-->
    ...
    >
      ...
      >
        <template slot="header" slot-scope="scope" >
          <template v-if="item.prop==='recomend_cluster'">
            <span>推荐加入集群</span>
            <el-tooltip popper-class="tooltip-width" effect="light">
              <div slot="content">
                <div>提示：</div>
                <span style="display: inline-block">1.主机匹配集群规则如下：</span>
                <span style="display: inline-block">主机的CPU、GPU、存储集群完全符合集群要求，且集群剩余容量大于等于1</span>
                <span style="display: inline-block">2.推荐默认显示符合要求的集群中剩余容量最大的（默认规则待定）</span>
              </div>
        ...
        <template #default="scope" v-else-if="item.prop==='recomend_cluster'">
          <el-button type="text" @click="updateRecommend(scope.row)">{{scope.row.recomend_cluster}}</el-button>
        </template>
      </el-table-column>
    </el-table>
    <el-pagination
    ...
    <template v-if="visible && !['physical_detail','upload','migrate','record','resource','update_attribute','business_test','remark','add_cluster'].includes(oper_type)">
    ...
    <template v-if="visible && oper_type==='add_cluster'">
      <add-cluster :visible.sync="visible" :rows="multi_rows.length>0 ? multi_rows : oper_host" @close="close"></add-cluster>
    </template>
    <custom-list-item
...
import AddCluster from "@/components/vmOp2/cluster/pod/host/addCluster.vue";
...
export default class HostList extends Vue{
  ...
  private menus= [
    ...
    {label: '导入集群',value:'add_cluster',disabled:false},
  ...
  private updateRecommend(obj){
    this.oper_type = 'update_recommend';
    this.oper_label = '修改推荐集群'
    this.visible = true;
    this.oper_host = [obj]
  }
  ...
  //改变点击行得选中状态
  private FnOperRow(row,column){
      if(column.property !== 'cluster_name'){
  ...
  private get_custom_columns(list) {
    ...
    this.custom_host.map((item:any)=>{
      if(!['cpu_model','gpu_model','bare_metal_name','power_status_name','machine_status_name','cluster_name'].includes(item.prop)) {
      ...
      if(['host_ip','out_band_address','gpu_count','ecs_count','cpu_rate','ram_rate','gpu_rage','machine_status_name','storage_cluster_name','power_status_name'].includes(item.prop)){
