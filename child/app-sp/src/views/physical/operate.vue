    ...
    >
        <div>
          ...
          <div v-if="notSelectVmList.length > 0" class="m-top10">
            <span class="tip_message">仅支持对状态为“锁定”、“存储异常”或“宕机”且无虚拟机运行的宿主机进行“设置维护”操作，有{{notSelectVmList.length}}台宿主机不满足要求</span>
          </div>
  ...
  private created() {
    ...
    if(this.oper_type === 'maintenance'){
      this.rows.forEach(row => {
        if (['lock','storage_error','crash'].includes(row.machine_status) && row.ecs_list.length === 0 ){
