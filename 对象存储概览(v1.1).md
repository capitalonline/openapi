...
| API参数名             | 单位 | 含义                   | 说明                                            |
| --------------------- | ---- |----------------------|-----------------------------------------------|
| StorageCapacitySize   | 字节 | 存储容量大小，支持周期：hour、day |                                               |
| IntranetSendBandwidth | bps  | 内网流出带宽，支持周期：minute | 每个时间点对应数值为该时间点前五分钟采集数据的平均值，如时间段内无内网流出流量则不显示该点 |
| InternetSendBandwidth | bps  | 公网流出带宽，支持周期：minute | 每个时间点对应数值为该时间点前五分钟采集数据的平均值，如时间段内无外网流出流量则不显示该点 |
...
--header 'Host':'metrics-test.oss-dal.cdsgss.com'
--header 'Content-Type': 'application/json'
--data '{
            "MetricKey": "StorageCapacitySize",
