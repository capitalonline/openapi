...
| GetBucketInfo    | 查询存储空间（Bucket）的相关信息     |
| GetBucketStat    | 获取指定存储空间（Bucket）的存储容量以及文件（Object）数量。     |
...
| UploadText       | 上传字符串到文本对象（Object）      |
| MultipartUpload  | 初始化，获得Upload ID（大文件上传步骤1）。 |
| UploadPart       | 上传分片（大文件上传步骤2）。           |
| InitiateMultipartUpload    | 完成上传（大文件上传步骤3）。             |
...
| DeleteBucketTags | 删除对象（Object）标签。                   |
