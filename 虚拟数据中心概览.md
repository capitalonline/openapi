...
* [虚拟数据中心相关](#虚拟数据中心相关)
    ...
    * [23.DescribeBYOIPSiteCreateOptions](#23DescribeBYOIPSiteCreateOptions)
    * [24.CreateBYOIPOneStep](#24CreateBYOIPOneStep)
    * [25.DeleteBYOIP](#25DeleteBYOIP)
    * [26.BroadcastBYOIP](#26BroadcastBYOIP)
    * [27.UndoBroadcastBYOIP](#27UndoBroadcastBYOIP)
    * [28.DescribeBYOIPList](#28DescribeBYOIPList)
    * [29.BatchDeletePublicIp](#29BatchDeletePublicIp)
...
def add_public_ip():
    ...
    param = {
        ...
        "ByoipId": "f3d2e84e-4f8d-11f1-becc-ca059aec6b14",
        "Number": "4"
    }
    url = get_signature(action, AK, AccessKeySecret, method, NETWORK_URL, param)
    res = requests.get(url)
...
def del_public_ip():
    ...
    method = "GET"
    param = {
        "SegmentId": "81b7c978-5285-11f1-9dba-46a2181d8ccf"
    }
    url = get_signature(action, AK, AccessKeySecret, method, NETWORK_URL, param)
    res = requests.get(url)
    result = json.loads(res.text)
...
**描述：** 查询创建 BYOIP 所需的节点、各节点下可选 AS 及对应公网（pipe）资源
**请求地址:** api.capitalonline.net/wan_service
**请求方法：POST**
...
| Data | dict | {} | 返回数据 |
| Data.SiteOptions | list | [] | 节点及创建选项列表 |
| Data.SiteOptions[].SiteId | string | | 节点 ID |
| Data.SiteOptions[].SiteName | string | | 节点名称 |
| Data.SiteOptions[].AsList | list | ["63199"] | 该节点可选 AS 号列表 |
| Data.SiteOptions[].AsnResources | list | [] | 按 AS 分组的公网资源 |
| Data.SiteOptions[].AsnResources[].Asn | string | 63199 | AS 号 |
| Data.SiteOptions[].AsnResources[].PipeList | list | [] | 可选公网 pipe 列表 |
| Data.SiteOptions[].AsnResources[].PipeList[].VdcId | string | | VDC ID |
| Data.SiteOptions[].AsnResources[].PipeList[].VdcName | string | | VDC 名称 |
| Data.SiteOptions[].AsnResources[].PipeList[].PipeId | string | | 公网 pipe ID |
| Data.SiteOptions[].AsnResources[].PipeList[].PipeName | string | | 公网 pipe 名称 |
...
def get_create_option():
    action = "DescribeBYOIPSiteCreateOptions"
    method = "POST"
    base_url = "http://api.capitalonline.net/wan_service/"
    url = get_signature(action, AK, AccessKeySecret, method, base_url)
    body = {}
    res = requests.post(url, json=body)
    return res.json()
```
                        ...
                        "PipeList": [
                            {
                                "VdcId": "vdc-uuid",
                                "VdcName": "示例 VDC",
                                "PipeId": "305c0ebc-10b2-4711-ae97-5e033aeb73fb",
                                "PipeName": "示例公网"
                            }
                        ]
                    }
...
| 名称 | 类型 | 是否必选 | 示例值 | 描述 |
| -------- | ------ | ---- | --- | ----------------- |
| PipeId | string | 是 | 305c0ebc-10b2-4711-ae97-5e033aeb73fb | 公网 pipe ID |
| SiteId | string | 是 | aae6922c-25e5-11ee-8783-be2af23df048 | 节点 ID |
| As | string | 是 | 63199 | AS 号 |
| Address | string | 是 | 164.52.66.0 | 公网段地址 |
| Mask | string | 是 | 24 | 掩码长度 |
| SubjectId | string | 否 | | 主体 ID |
| ProjectId | string | 否 | 0-0 | 项目 ID |
| file | file | 是 | LOA.pdf | LOA 授权 PDF，字段名固定为 `file` |
...
| Data.ByoipId | string | | 创建的 BYOIP 资源 ID |
...
def create_one():
    ...
    with open("LOA.pdf", "rb") as f:
        files = {"file": ("LOA.pdf", f, "application/pdf")}
        res = requests.post(url, data=form, files=files)
    return res.json()
```
...
| 名称 | 类型 | 是否必选 | 示例值 | 描述 |
| -------- | ------ | ---- | --- | ----------------- |
| ByoipId | string | 是 | 3179d63e-3d33-11f1-a02b-4ace70fb9336 | BYOIP 资源 ID |
...
| Data | dict | {} | 返回数据 |
| Data.TaskId | string | | 异步任务 ID |
| Data.ByoipId | string | | BYOIP 资源 ID |
...
def broadcast():  
    ...
    method = "POST"  
    base_url = 'http://cdsapi-gateway.gic.test/wan_service/'  
    url = get_signature(action, AK, AccessKeySecret, method, base_url)  
    param = {  
        "ByoipId":"9c52d91a-2cc6-11f1-ab40-1c697a6f532d"  
    }  
...
{
  "Data": {
    "TaskId": "509b46e4-541a-11f1-a834-eec67fba1cfa",
    "ByoipId": "d59ba704-5419-11f1-892b-eec67fba1cfa"
  },
  "Code": "OK",
  "Message": "任务下发成功",
  "Success": true,
  "RequestId": "86471b7f76054c8dadad2b091532ebbb"
}
```
...
{
  "Data": {
    "TaskId": "ceb1fd9a-5364-11f1-af1f-025fdb53e56a",
    "ByoipId": "9c52d91a-2cc6-11f1-ab40-1c697a6f532d"
  },
  "Code": "OK",
  "Message": "任务下发成功",
  "Success": true,
  "RequestId": "d6c4a8999413478fa5991ec2c93969a0"
}
```
...
def undo_broadcast():
    ...
    url = get_signature(action, AK, AccessKeySecret, method, base_url)
    body = {
        "ByoipId": "3179d63e-3d33-11f1-a02b-4ace70fb9336"
    }
    res = requests.post(url, json=body)
    return res.json()
```
...
**描述：** 分页查询客户 BYOIP 列表，支持按公网 pipe、关键字等过滤。  
**请求地址:** api.capitalonline.net/wan_service  
**请求方法：POST**
...
| 名称 | 类型 | 是否必选 | 示例值 | 描述 |
| -------- | ------ | ---- | --- | ----------------- |
| Keyword | string | 否 | | 关键字（ID / 地址） |
| PipeId | string | 否 | 305c0ebc-10b2-4711-ae97-5e033aeb73fb | 按公网 pipe 过滤 |
| CreateSegment | bool | 否 | true | 为 true 时仅返回已广播、可用于创建网段的记录 |
| Page | int | 否 | 1 | 页码，默认 1 |
| PageSize | int | 否 | 20 | 每页条数，默认 20 |
| ShowAll | bool | 否 | true | 为 true 时不分页，返回全部 |
| AreaId | string | 否 | CN | 区域标识 |
...
| Data.ByoipList | list | [] | BYOIP 列表 |
| Data.ByoipList[].Id | string | | BYOIP ID |
| Data.ByoipList[].PipeId | string | | 公网 pipe ID |
| Data.ByoipList[].Address | string | 164.52.66.0 | 网段地址 |
| Data.ByoipList[].Mask | int | 24 | 掩码 |
| Data.ByoipList[].AddressStr | string | 164.52.66.0/24 | 地址/掩码展示 |
| Data.ByoipList[].Asn | string | 63199 | AS 号 |
| Data.ByoipList[].Status | string | broadcasted | 状态英文 |
| Data.ByoipList[].StatusZh | string | 已广播 | 状态中文 |
| Data.ByoipList[].VdcId | string | | VDC ID |
| Data.ByoipList[].VdcName | string | | VDC 名称 |
| Data.ByoipList[].PipeName | string | | 公网名称 |
| Data.ByoipList[].SiteId | string | | 节点 ID |
| Data.ByoipList[].SiteName | string | | 节点名称 |
| Data.ByoipList[].IpNumList | list | [4, 8, 16] | 可选 IP 数量档位 |
| Data.ByoipList[].VdcProjectId | string | 0-0 | VDC 项目 ID |
...
```json
{
    ...
    "Data": {
        ...
        "ByoipList": [
            {
                "Id": "3179d63e-3d33-11f1-a02b-4ace70fb9336",
                "PipeId": "305c0ebc-10b2-4711-ae97-5e033aeb73fb",
                "Address": "164.52.66.0",
                "Mask": 24,
                "AddressStr": "164.52.66.0/24",
                "Asn": "63199",
                "Status": "broadcasted",
                "StatusZh": "已广播",
                "VdcId": "vdc-uuid",
